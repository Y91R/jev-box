...
Плагин поставляет два скилла — копии глобальных `analyst-reviewer` и `system-analyst` с дешёвым проходом Jev:
