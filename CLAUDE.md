...
Логика — чистые модули без `$`: `config.ts` (чтение и проверка конфига по FR-4), `candidates.ts` (отбор по окну контекста), `jev.ts` (запрос к Jev и разбор ответа, общие для обоих провайдеров), `classify.ts` (вызов с таймаутом, коды отказов, лог), `turns.ts` (решение на ход и правило о резервной модели движка), `debuglog.ts` (отладочный лог: буфер JSONL, файл переписывается целиком — у `$.fs` нет дозаписи). `register.ts` только связывает события с этими модулями.
